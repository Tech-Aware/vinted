...
def test_render_pull_tommy_femme_marks_pima_cotton_as_premium() -> None:
    template = ListingTemplateRegistry().get_template("template-pull-tommy-femme")
    ...
    fields = ListingFields(
        ...
        sku="PTF42",
        knit_pattern="pois",
        feature_notes="100% Pima Cotton / 100% Coton Pima",
    )
...
def test_render_pull_tommy_femme_detects_spaced_pima_letters() -> None:
    template = ListingTemplateRegistry().get_template("template-pull-tommy-femme")
    ...
    fields = ListingFields(
        ...
        sku="PTF66",
        knit_pattern="",
        feature_notes="100% P I M A   C O T O N",
    )
...
def test_generate_listing_ignores_numeric_polaire_sku(
    monkeypatch: pytest.MonkeyPatch, raw_sku: str, brand: str
) -> None:
    template = ListingTemplateRegistry().get_template("template-polaire-outdoor")
